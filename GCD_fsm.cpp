...
SC_MODULE(GCD_Controller) {
    ...
    sc_in<sc_uint<6>> a_in, b_in;
    sc_out<sc_uint<6>> gcd_out;
    
    sc_signal<sc_uint<6>> x, y;
    sc_signal<bool> x_greater, x_equal, x_smaller;
    sc_signal<sc_uint<6>> x_sub_y, y_sub_x;
    sc_signal<int> state;

    enum States { IDLE, LOAD, COMPARE, SUBTRACT_X, SUBTRACT_Y, DONE };
...
SC_MODULE(Testbench) {
    sc_signal<sc_uint<6>> a_sig, b_sig, gcd_sig;
    sc_clock clk_sig;
    ...
    void run_tests() {
        ...
        for (test_index = 0; test_index < TEST_SIZE; ++test_index) {
            ...
            wait(50, SC_NS);
            std::cout << "GCD(" << test_cases[test_index].first << ", "
                      << test_cases[test_index].second << ") = "
                      << gcd_sig.read() << std::endl;
        }
        sc_stop();
    }
